...
export const usePlaceStore = create((set, get) => ({
  ...
  getAllPlaces: async (filters = {}) => {
    ...
    try {
      ...
      const finalUrl = `/api/places/explore?${queryParams.toString()}`;
  ...
  getTrendingPlaces: async () => {
    ...
    try {
      const res = await axiosInstance.get("/api/places/trending");
  ...
  getNewestPlaces: async () => {
    ...
    try {
      const res = await axiosInstance.get("/api/places/newest");
  ...
  getPlaceBySlug: async (slug) => {
    ...
    try {
      const res = await axiosInstance.get(`/api/places/${slug}`);
  ...
  getMyContributions: async () => {
    ...
    try {
      const res = await axiosInstance.get("/api/places/my-contributions");
  ...
  getMyBookmarks: async (limit = 10, skip = 0, append = false) => {
    ...
    try {
      const res = await axiosInstance.get(`/api/places/my-bookmarks?limit=${limit}&skip=${skip}`);
      if (append) {
  ...
  addBookmark: async (placeId, placeData = null) => {
    ...
    try {
      const res = await axiosInstance.post(`/api/places/my-bookmarks/${placeId}`);
  ...
  removeBookmark: async (placeId) => {
    ...
    set((state) => ({ loadingBookmarkIds: new Set([...state.loadingBookmarkIds, placeId]) }));
    try {
      const res = await axiosInstance.delete(`/api/places/my-bookmarks/${placeId}`);
