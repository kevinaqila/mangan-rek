...
export const addPlace = async(req, res) => {
    ...
    try {
        ...
        if (files.galleryImages) {
            ...
            const uploadPromises = files.galleryImages.map(file => cloudinary.uploader.upload(file.path, { folder: "mangan_rek_places" }));
...
export const getTrendingPlaces = async(req, res) => {
    try {
        ...
        const trendingPlaces = await Place.find({ '_id': { $in: placeIds } })
            .populate("category")
...
export const getNewestPlaces = async(req, res) => {
    try {
        const newestPlaces = await Place.find().sort({ createdAt: -1 }).limit(5).populate("category").populate("createdBy", "fullName profilePic");
...
export const getPlaceBySlug = async(req, res) => {
    try {
        const place = await Place.findOne({ slug: req.params.slug }).populate("category").populate("createdBy", "fullName profilePic");
        if (!place) {
...
export const updatePlace = async(req, res) => {
    try {
        ...
        if (files && files.mainImage) {
            ...
            const mainImageResult = await cloudinary.uploader.upload(files.mainImage[0].path, { folder: "mangan_rek_places" });
...
export const getMyContributions = async(req, res) => {
    try {
        ...
        const contributions = await Place.find({ createdBy: userId }).sort({ createdAt: -1 }) // Urutkan dari yang terbaru
